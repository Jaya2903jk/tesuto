...
if (!(isset($_SESSION['student_username']))) {
    ...
    include_once 'dbConnection.php';
    echo '<span class="pull-right top title1" ><span style="color:white"><span class="glyphicon glyphicon-user" aria-hidden="true"></span>&nbsp;&nbsp;&nbsp;&nbsp;Hello,</span><span class="log log1" style="color:lightyellow">' . $name . '&nbsp;&nbsp;|&nbsp;&nbsp;<a href="logout.php?q=account_odd.php" style="color:lightyellow"><span class="glyphicon glyphicon-log-out" aria-hidden="true"></span>&nbsp;Logout</button></a></span>';
}
...
if($_SESSION['6e447159fg']=='6e447159fg'){
 echo "<script>alert('Dear Candidate,Your Online Examination is successfully completed');</script>";
    ...
    echo '<div class="panel" id="exam_code_check"><table class="table table-striped title1 "  style="vertical-align:middle">
     <form action="account_odd.php?q=1" method="post">
...
if($count==1){
    $_SESSION["code"]=$code_exam;
     ...
     header("location:account_odd.php?q=99");
}
...
if($rowcount == 0) {
  ...
  <td style="vertical-align:middle"><b><a href="account_odd.php?q=quiz&step=2&eid='.$eid.'&n=1&t=' . $total . '&start=start" class="btn" style="color:#FFFFFF;background:darkgreen;font-size:12px;padding:7px;padding-left:10px;padding-right:10px"><span class="glyphicon glyphicon-new-window" aria-hidden="true"></span>&nbsp;<span><b>Start</b></span></a></b></td>

  </tr>';
  // <a href="account_odd.php?q=quiz&step=2&eid='.$eid.'&n='.$_SESSION['n'].'&t=' . $total . '&start=start" class="btn" style="color:#FFFFFF;background:darkgreen;font-size:12px;padding:7px;padding-left:10px;padding-right:10px"><span class="glyphicon glyphicon-new-window" aria-hidden="true"></span>&nbsp;<span><b>Start</b></span></a>
  // important----------------up
  ...
  $_SESSION['total']=$total;
         ...
         else
          {
            ...
            $_SESSION['continue']='61454rff221';
                ...
                <td style="vertical-align:middle;display:none;">' . $correct * $total . '</td>
                <td style="vertical-align:middle">' . $time . '&nbsp;min</td>
  <td style="vertical-align:middle"><b><a href="account_odd.php?q=quiz&step=2&eid=' . $eid . '&n='.$nq.'&t=' . $total . '&start=start" class="btn" style="margin:0px;background:darkorange;color:white">&nbsp;<span class="title1"><b>Continue</b></span></a></b></td></tr>';
         // <td style="vertical-align:middle"><b><a href="account_odd.php?q=quiz&step=2&eid=' . $eid . '&n=1&t=' . $total . '&start=start" class="btn" style="margin:0px;background:darkorange;color:white">&nbsp;<span class="title1"><b>Continue</b></span></a></b></td></tr>';
    ...
    <td style="vertical-align:middle;display:none;">' . $correct * $total . '</td>
                <td style="vertical-align:middle">' . $time . '&nbsp;min</td>
  <td style="vertical-align:middle"><b><a href="account_odd.php?q=1" class="btn disabled" style="margin:0px;background:darkred;color:white">&nbsp;<span class="title1"><b>Exam Over</b></span></a></b></td></tr>';
            }
        }
    }
    $c = 0;
...
if (@$_GET['q'] == 'quiz' && @$_GET['step'] == 2 && isset($_SESSION['6e447159425d2d']) && $_SESSION['6e447159425d2d'] == "6e447159425d2d" && isset($_GET['endquiz'])== 'end') 
{
    ...
    $q = mysqli_query($con, "UPDATE tesuto_history SET status='finished' WHERE username='$_SESSION[student_username]' AND eid='$_GET[eid]' ") or die('Error197');
        $q = mysqli_query($con, "SELECT * FROM tesuto_history WHERE eid='$_GET[eid]' AND username='$_SESSION[username]'") or die('Error156');
            ...
            header('location:http://localhost/veltech/exam/tesuto/student/account_odd.php?q=1');
   ...
   // }
    ...
    if (mysqli_num_rows($q) > 0) {
        ...
        if ($status == "ongoing" && $remaining > 0 && $qstatus == "enabled") {
            ...
            header('location:account_odd.php?q=quiz&step=2&eid=' . $_GET[eid] . '&n=' . $_GET[n] . '&t=' . $_GET[t]);
        ...
        $q = mysqli_query($con, "SELECT * FROM tesuto_history WHERE eid='$_GET[eid]' AND username='$_SESSION[student_username]'") or die('Error156');
            ...
            header('location:account_odd.php?q=result&eid=' . $_GET[eid]);
        }
        ...
        header('location:account_odd.php?q=quiz&step=2&eid=' . $_GET["eid"] . '&n=' . $_GET["n"] . '&t=' . $_GET["t"]);
       // q=quiz&step=2&eid=&n=' . $_GET["n"] . '&t=' . $_GET["t"]
    }
}
...
function end(){
  ...
  if(data=="yes"||data=="YES"){
    window.location ="account_odd.php?q=quiz&step=2&eid=' . $_GET["eid"] . '&n=' . $_GET["n"] . '&t=' . isset($_GET["total"]) . '&endquiz=end";
...
//                 echo '</pre>';
  ...
  $i++;
    ...
    echo '<td><a class="' . $c . '" href="account_odd.php?q=quiz&step=2&eid=' . $eid . '&n=' . $count_org . '&t=' . $total . '" class="text-light" style="color:white;" role="button"><b>' . $count . '-</b><b>' . $show . '</b></a></td>';
...
// //         echo '</tr><tr>';
// //     }
//  echo '<td><a class="'.$c.'" href="account_odd.php?q=quiz&step=2&eid=' . $eid . '&n=' . $count . '&t='.$total.'" class="text-light" style="color:white;" role="button"><b>'.$count.'-</b><b>'.$show.'</b></a></td>';
...
exit();
          ...
          // echo "$sn";
           // ------------------------//
// for ($i = $total; $i >= 1; $i--) {
//     $q     = mysqli_query($con, "SELECT * FROM tesuto_questions WHERE eid='$eid' AND sn='$i' And branch='$branch' ");
//              if ($q) {
...
// http://localhost/veltech/exam/tesuto/admin/uploads/qs-1096935-Capture.JPG.png         
       //https://apps.veltech.edu.in/exams/tesuto/admin/uploads/qs-280736-16q.JPG
//https://apps.veltech.edu.in/exams/tesuto/admin/uploads/;
...
echo '<b><pre style="background-color:white"><div style="font-size:20px;font-weight:bold;font-family:calibri;margin:10px">' . $sn . ' :
 <img src="' . $url . '"> </div></pre></b>';
              ...
              // ------------------------//
            echo '<form id="qform" action="update_odd.php?q=quiz&step=2&eid=' . $eid . '&n=' . $sn . '&t=' . $total . '&qid=' . $qid . '" method="POST"  class="form-horizontal">
<br />';
            ...
            if ($_GET["t"] > $_GET["n"] && $_GET["n"] != 1) {
                ...
                echo '<br /><a href="account_odd.php?q=quiz&step=2&eid=' . $eid . '&n=' . ($sn + 1) . '&t=' . $total . '" class="btn btn-primary" style="height:30px"><span class="glyphicon glyphicon-arrow-left" aria-hidden="true"  style="font-size:12px"></span></a>&nbsp;&nbsp;&nbsp;&nbsp;<button type="submit" class="btn btn-success" disabled="true" id="sbutton" style="height:30px"><span class="glyphicon glyphicon-lock" style="font-size:12px" aria-hidden="true"></span><font style="font-size:12px;font-weight:bold"> Lock</font></button>&nbsp;&nbsp;&nbsp;&nbsp;';
                ...
                echo'&nbsp;&nbsp;&nbsp;&nbsp;<a href="account_odd.php?q=quiz&step=2&eid=' . $eid . '&n=' . ($sn - 1) . '&t=' .$total.'" class="btn btn-primary" style="height:30px"><span class="glyphicon glyphicon-arrow-right" aria-hidden="true"  style="font-size:12px"></span></a></form><br><br>';
                // q=quiz&step=2&eid=Exam_230001&n=10&t=10&v=sum
                 echo '<br/>&nbsp;&nbsp;&nbsp;&nbsp;<a class="btn btn-default" href="account_odd.php?q=quiz&step=2&eid=' . $eid .'&n=' .$total . '&t='. $total . '&v=sum" role="button">View Summary</a>  &nbsp;&nbsp;&nbsp;&nbsp;</form><br><br>';
...
echo 'now last';
                ...
                echo '<br/><button type="submit" class="btn btn-primary" disabled="true" id="sbutton" style="height:30px"><span class="glyphicon glyphicon-lock" style="font-size:12px" aria-hidden="true"></span><font style="font-size:12px;font-weight:bold"> Lock</font></button>&nbsp;&nbsp;&nbsp;&nbsp;<a class="btn btn-default" href="account_odd.php?q=quiz&step=2&eid=' . $eid .'&n=' .$total . '&t='. $total . '&v=sum" role="button">View Summary</a>  &nbsp;&nbsp;&nbsp;&nbsp;</form><br><br>';
         ...
         //else if ($_GET["t"] > $_GET["n"] && $_GET["n"] == 1) 
        else if ($_GET["t"] == $_GET["n"])
            {
                ...
                // <button type="button" class="btn btn-default" onclick="frmreset()" style="height:30px"></span><font style="font-size:12px;font-weight:bold">Reset</font></button>';
                echo '&nbsp;&nbsp;&nbsp;&nbsp;<a href="account_odd.php?q=quiz&step=2&eid=' . $eid . '&n=' . ($sn - 1) . '&t=' . $total . '" class="btn btn-primary" style="height:30px"><span class="glyphicon glyphicon-arrow-right" aria-hidden="true"  style="font-size:12px">Next</span></a></form><br><br>';
...
//    //  <a href="account.php?q=quiz&step=2&eid=' . $eid . '&n=' . $count . '&t='.$total.'" class="text-light" style="color:white;"><b>'.$count.'-</b><b>'.$show.'</b></a>
//    //  </button>';
// } // for loop  second
...
<tr><td style="vertical-align:middle"><b>S.N.</b></td><td style="vertical-align:middle"><b>Quiz</b></td><td style="vertical-align:middle"><b>Total Questions</b></td><td style="vertical-align:middle"><b>Right</b></td><td style="vertical-align:middle"><b>Wrong<b></td><td style="vertical-align:middle"><b>Unattempted<b></td><td style="vertical-align:middle"><b>Score</b></td><td style="vertical-align:middle"><b>Action<b></td></tr>';
    ...
    while ($row = mysqli_fetch_array($q)) {
        ...
        echo '<tr><td style="vertical-align:middle">' . $c . '</td><td style="vertical-align:middle">' . $title . '</td><td style="vertical-align:middle">' . $total . '</td><td style="vertical-align:middle">' . $r . '</td><td style="vertical-align:middle">' . $w . '</td><td style="vertical-align:middle">' . ($total - $r - $w) . '</td><td style="vertical-align:middle">' . $s . '</td><td style="vertical-align:middle"><b><a href="account_odd.php?q=result&eid=" class="btn" style="margin:0px;background:darkred;color:white">&nbsp;<span class="title1"><b>View Result1</b></td></tr>';
    }
    echo '</table></div>';
}
if (@$_GET['q'] == 3) {
...
<tr><td style="vertical-align:middle"><b>Rank</b></td><td style="vertical-align:middle"><b>Name</b></td><td style="vertical-align:middle"><b>Branch</b></td><td style="vertical-align:middle"><b>Username</b></td><td style="vertical-align:middle"><b>Score</b></td></tr>';
    ...
    if($show == 1 && $total==1){
        ...
        $i = 1;
        while($i<=$total){
            echo '<td style="vertical-align:middle;text-align:center"><a style="font-size:14px;font-family:typo;font-weight:bold" href="account_odd.php?q=3&show='.$i.'">&nbsp;'.$i.'&nbsp;</a></td>';
            $i++;
        }
        echo '<td style="vertical-align:middle;text-align:center"><a style="font-size:14px;font-family:typo;font-weight:bold" href="account_odd.php?q=3&show='.($show+1).'">&nbsp;>>&nbsp;</a></td>';
    }
