...
//     $movetempnat = move_uploaded_file($ntemptransfer1, $folder1.$qns);
             ...
             $qns  = addslashes($_POST['qns'. $i]);
              //print_r($qns);die();
            $q3   = mysqli_query($con, "INSERT INTO tesuto_questions VALUES  (NULL,'$eid','$qid','$qns' , '$ch' , '$i')") or die();
            $oaid = uniqid();
            $obid = uniqid();
            $ocid = uniqid();
            $odid = uniqid();
    ...
    if (mysqli_num_rows($q) > 0) {
        ...
        if ($status == "ongoing" && $remaining > 0 && $qstatus == "enabled") {
           ...
           $time_stamp = date("Y-m-d H:i:s");
            ...
            header("location:account_even.php?q=quiz&step=2&eid=$eid&n=$sn&t=$total") or die('Error152');
...
if (@$_GET['q'] == 'quiz' && @$_GET['step'] == 2 && isset($_SESSION['6e447159425d2d']) && $_SESSION['6e447159425d2d'] == "6e447159425d2d" && (!isset($_POST['ans'])) && (isset($_GET['delanswer'])) && $_GET['delanswer'] == "delanswer") {
    ...
    if (mysqli_num_rows($q) > 0) {
        ...
        if ($status == "ongoing" && $remaining > 0 && $qstatus == "enabled") {
            ...
            header('location:account_even.php?q=quiz&step=2&eid=' . $_GET[eid] . '&n=' . $_GET[n] . '&t=' . $total);
