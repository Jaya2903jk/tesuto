...
if (isset($_SESSION['key'])) {
    if (@$_GET['q'] == 'rmquiz' && $_SESSION['key'] == '54585c506829293a2d4c3b68543b316e2e7a2d277858545a36362e5f39') {
        $eid = @$_GET['eid'];
        $result = mysqli_query($con, "SELECT * FROM tesuto_questions WHERE eid='$eid' ") or die('Error');
        ...
        $r3 = mysqli_query($con, "DELETE FROM tesuto_questions WHERE eid='$eid' ") or die('Error');
...
if (isset($_SESSION['key'])) {
    if (@$_GET['q'] == 'addquiz' && $_SESSION['key'] == '54585c506829293a2d4c3b68543b316e2e7a2d277858545a36362e5f39') {
       ...
       // $name    = ucwords(strtolower($name));
        ...
        $status  = "disabled";
       // $id      = uniqid();
        $get=mysqli_query($con, "SELECT eid FROM tesuto_quiz  ORDER BY `tesuto_quiz`.`eid` DESC LIMIT 1;");
        $get1=mysqli_fetch_array($get);
        $row_count = mysqli_num_rows($get);
        $count1 = $get1['eid'];
...
if (isset($_SESSION['key'])) {
 ...
 $get_branch=mysqli_fetch_array(mysqli_query($con, "SELECT * FROM tesuto_quiz where eid='$eid'"));
...
// for ($i = 1; $i <= $n; $i++)
  ...
  {
    //echo $i;
    $get=mysqli_query($con, "SELECT qid FROM tesuto_questions  ORDER BY `tesuto_questions`.`qid` DESC LIMIT 1;");
    $get1=mysqli_fetch_array($get);
    $row_count = mysqli_num_rows($get);
    $count1 = $get1['qid'];
    if($row_count == 0)
    ...
    $movetempnat = move_uploaded_file($ntemptransfer1, $folder1.$qns);
            ...
            $q3   = mysqli_query($con, "INSERT INTO tesuto_questions VALUES  (NULL,'$eid','$qid','$qns','$branch','$ch','$serialno')") or die();
 ...
 $answer = $answer +1;
            ...
            $qans = mysqli_query($con, "INSERT INTO tesuto_answer VALUES  (NULL,'$qid','$ansid')");
              ...
              ///////// shuffle/////
      // Inner loop (decrement from the middle)
    //$middleValue = floor($i / 2) + 1;
   // for ($j = $middleValue; $j >= 1; $j--) {
      //$q4   = mysqli_query($con,"INSERT INTO tesuto_questions_b VALUES  (NULL,'$eid','$qid','$qns[$j]','$branch','$ch','$serialno_dummy')") or die();
       //$serialno_dummy = $serialno_dummy +1;
    ///////}
     // print_r($qans);die();
        }//outer loop
...
if (@$_GET['q'] == 'quiz' && @$_GET['step'] == 2 && isset($_SESSION['6e447159425d2d']) && $_SESSION['6e447159425d2d'] == "6e447159425d2d" && isset($_POST['ans']) && (!isset($_GET['delanswer']))) {
    ...
    if (mysqli_num_rows($q) > 0) {
        $q = mysqli_query($con, "SELECT * FROM tesuto_history WHERE username='$_SESSION[username]' AND eid='$_GET[eid]' ") or die('Error197');
        while ($row = mysqli_fetch_array($q)) {
            $time   = $row['timestamp'];
            $status = $row['status'];
        }
        ...
        if ($status == "ongoing" && $remaining > 0 && $qstatus == "enabled") {
            $q = mysqli_query($con, "SELECT * FROM tesuto_user_answer WHERE eid='$_GET[eid]' AND username='$_SESSION[username]' AND qid='$qid' ") or die('Error115');
            while ($row = mysqli_fetch_array($q)) {
                $prevans = $row['ans'];
            }
            ...
            $q = mysqli_query($con, "SELECT * FROM tesuto_user_answer WHERE username='$_SESSION[username]' AND eid='$_GET[eid]' AND qid='$qid' ") or die('Error1977');
            if (mysqli_num_rows($q) != 0) {
                $q = mysqli_query($con, "UPDATE tesuto_user_answer SET ans='$ans' WHERE username='$_SESSION[username]' AND eid='$_GET[eid]' AND qid='$qid' ") or die('Error197');
            } else {
                $q = mysqli_query($con, "INSERT INTO tesuto_user_answer VALUES(NULL,'$qid','$ans','$ansid','$_GET[eid]','$_SESSION[username]')");
            }
            ...
            $q = mysqli_query($con, "UPDATE tesuto_history SET status='finished' WHERE username='$_SESSION[username]' AND eid='$_GET[eid]' ") or die('Error197');
        $q = mysqli_query($con, "SELECT * FROM tesuto_history WHERE eid='$_GET[eid]' AND username='$_SESSION[username]'") or die('Error156');
                ...
                if($scorestatus=="false"){
                    $q = mysqli_query($con, "UPDATE tesuto_history SET score_updated='true' WHERE username='$_SESSION[username]' AND eid='$_GET[eid]' ") or die('Error197');
