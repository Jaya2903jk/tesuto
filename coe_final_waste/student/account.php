...
if (@$_GET['q'] == 2)
    echo 'class="active"';
?>>
<!-- <a href="account.php?q=2"><span class="glyphicon glyphicon-list-alt" aria-hidden="true"></span>&nbsp;My History</a> -->
</li>
    <li <?php
if (@$_GET['q'] == 3)
    echo 'class="active"';
?>>
<!-- <a href="account.php?q=3"><span class="glyphicon glyphicon-stats" aria-hidden="true"></span>&nbsp;Leaderboard</a> -->
</li></ul>
...
if (@$_GET['q']==99){
    ...
    echo '<div class="panel" id="exam_code_check"><table class="table table-striped title1 "  style="vertical-align:middle">
     <form action="account.php?q=1" method="post">
...
{
$code=$_POST['ex_code'];
$query=mysqli_query($con,"SELECT * from exam_code where ex_code='$code'");
...
if($count==1){
   ...
   {
    //print_r("SELECT * FROM quiz WHERE status = 'enabled' And title='$branch'  ORDER BY date DESC");
    $result = mysqli_query($con, "SELECT * FROM quiz WHERE status = 'enabled' And title='$branch'  ORDER BY date DESC") or die('Error');
...
<tr><td style="vertical-align:middle"><b>S.N.</b></td><td style="vertical-align:middle"><b>Name</b></td><td style="vertical-align:middle"><b>Total question</b></td><td style="vertical-align:middle"><b>Correct Answer</b></td><td style="vertical-align:middle"><b>Wrong Answer</b></td><td style="vertical-align:middle"><b>Total Marks</b></td><td style="vertical-align:middle"><b>Time limit</b></td><td style="vertical-align:middle"><b>Action</b></td></tr>';
  ...
  $_SESSION['total']=$total;
          ...
          {
            $q = mysqli_query($con, "SELECT * FROM history WHERE username='$_SESSION[student_username]' AND eid='$eid' ") or die('Error197');
  ...
  <td style="vertical-align:middle"><b><a href="account.php?q=quiz&step=2&eid=' . $eid . '&n=1&t=' . $total . '&start=start" class="btn" style="margin:0px;background:darkorange;color:white">&nbsp;<span class="title1"><b>Continue</b></span></a></b></td></tr>';
                ...
                echo '<tr style="color:darkgreen"><td style="vertical-align:middle">' . $c++ . '</td><td style="vertical-align:middle">' . $title . '&nbsp;<span title="This quiz is already solve by you" class="glyphicon glyphicon-ok" aria-hidden="true"></span></td><td style="vertical-align:middle">' . $total . '</td><td style="vertical-align:middle">+' . $correct . '</td><td style="vertical-align:middle">-' . $wrong . '</td><td style="vertical-align:middle">' . $correct * $total . '</td><td style="vertical-align:middle">' . $time . '&nbsp;min</td>
  <td style="vertical-align:middle"><b><a href="account.php?q=1" class="btn disabled" style="margin:0px;background:darkred;color:white">&nbsp;<span class="title1"><b>View Result</b></span></a></b></td></tr>';
            }
        }
    }
    $c = 0;
...
if (@$_GET['q'] == 'quiz' && @$_GET['step'] == 2 && isset($_SESSION['6e447159425d2d']) && $_SESSION['6e447159425d2d'] == "6e447159425d2d" && isset($_GET['endquiz'])== 'end') 
{
    ...
    $q = mysqli_query($con, "UPDATE history SET status='finished' WHERE username='$_SESSION[student_username]' AND eid='$_GET[eid]' ") or die('Error197');
        $q = mysqli_query($con, "SELECT * FROM history WHERE eid='$_GET[eid]' AND username='$_SESSION[username]'") or die('Error156');
                while ($row = mysqli_fetch_array($q)) {
                 ...
                 if($scorestatus=="false"){
                    $q = mysqli_query($con, "UPDATE history SET score_updated='true' WHERE username='$_SESSION[student_username]' AND eid='$_GET[eid]' ") or die('Error197');
                ...
                header('location:http://localhost/coe_final /student/account.php?q=1');
...
if (@$_GET['q'] == 'quiz' && @$_GET['step'] == 2 && isset($_GET['start']) && $_GET['start'] == "start" && (!isset($_SESSION['6e447159425d2d'])))
 {
   ...
   // }
    ...
    if (mysqli_num_rows($q) > 0) {
        //echo 'hi';die();
        $q = mysqli_query($con, "SELECT * FROM history WHERE username='$_SESSION[student_username]' AND eid='$_GET[eid]' ") or die('Error197');
        while ($row = mysqli_fetch_array($q)) {
            $timel  = $row['timestamp'];
            $status = $row['status'];
        }
        ...
        if ($status == "ongoing" && $remaining > 0 && $qstatus == "enabled") {
            ...
            header('location:account.php?q=quiz&step=2&eid=' . $_GET[eid] . '&n=' . $_GET[n] . '&t=' . $_GET[t]);
                ...
                $q = mysqli_query($con, "UPDATE history SET status='finished' WHERE username='$_SESSION[student_username]' AND eid='$_GET[eid]' ") or die('Error197');
        $q = mysqli_query($con, "SELECT * FROM history WHERE eid='$_GET[eid]' AND username='$_SESSION[student_username]'") or die('Error156');
                while ($row = mysqli_fetch_array($q)) {
                    $s = $row['score'];
                    $scorestatus = $row['score_updated'];
                }
...
//         echo '</tr><tr>';
//     }
 echo '<td><a class="'.$c.'" href="account.php?q=quiz&step=2&eid=' . $eid . '&n=' . $count . '&t='.$total.'" class="text-light" style="color:white;" role="button"><b>'.$count.'-</b><b>'.$show.'</b></a></td>';
...
exit();
            ...
            $q     = mysqli_query($con, "SELECT * FROM questions WHERE eid='$eid' AND sn='$sn' And branch='$branch' ");
            echo '<div class="panel" style="margin-right:5%;margin-left:5%;margin-top:10px;border-radius:10px">';
            while ($row = mysqli_fetch_array($q)) {
                $qns = stripslashes($row['qns']);
                $qid = $row['qid'];
                echo '<b><pre style="background-color:white"><div style="font-size:20px;font-weight:bold;font-family:calibri;margin:10px">' . $sn . ' : <img src="http://localhost/coe_final/admin/uploads/' . $qns . '"> </div></pre></b>';
            }
            echo '<form id="qform" action="update.php?q=quiz&step=2&eid=' . $eid . '&n=' . $sn . '&t=' . $total . '&qid=' . $qid . '" method="POST"  class="form-horizontal">
<br />';
            $q = mysqli_query($con, "SELECT * FROM user_answer WHERE qid='$qid' AND username='$_SESSION[student_username]' AND eid='$_GET[eid]'") or die("Error222");
            if (mysqli_num_rows($q) > 0){
...
//print_r($ans);
            ...
            if ($_GET["t"] > $_GET["n"] && $_GET["n"] != 1) {
                ...
                echo '<br/><button type="submit" class="btn btn-primary" disabled="true" id="sbutton" style="height:30px"><span class="glyphicon glyphicon-lock" style="font-size:12px" aria-hidden="true"></span><font style="font-size:12px;font-weight:bold"> Lock</font></button>&nbsp;&nbsp;&nbsp;&nbsp;<a class="btn btn-default" href="account.php?q=quiz&step=2&eid=' . $eid .'&n=' .$n . '&t='. $total . '&v=sum" role="button">View Summary</a>  &nbsp;&nbsp;&nbsp;&nbsp;</form><br><br>';

            } else if ($_GET["t"] > $_GET["n"] && $_GET["n"] == 1) {

                echo '<br />&nbsp;&nbsp;&nbsp;&nbsp;<button type="submit" class="btn btn-default" disabled="true" id="sbutton" style="height:30px"><span class="glyphicon glyphicon-lock" style="font-size:12px" aria-hidden="true"></span><font style="font-size:12px;font-weight:bold"> Lock<font></button>&nbsp;&nbsp;&nbsp;&nbsp;<button type="button" class="btn btn-default" onclick="frmreset()" style="height:30px"></span><font style="font-size:12px;font-weight:bold">Reset</font></button>&nbsp;&nbsp;&nbsp;&nbsp;<a href="account.php?q=quiz&step=2&eid=' . $eid . '&n=' . ($sn + 1) . '&t=' . $total . '" class="btn btn-primary" style="height:30px"><span class="glyphicon glyphicon-arrow-right" aria-hidden="true"  style="font-size:12px">Next</span></a></form><br><br>';
            ...
            $q = mysqli_query($con, "SELECT * FROM user_answer WHERE eid='$_GET[eid]' AND username='$_SESSION[student_username]'") or die("Error222a");
            $i = 1;
            while ($row = mysqli_fetch_array($q)) {
...
<center><h1 class="title" style="color:#660033">Result</h1><center><br /><table class="table table-striped title1" style="font-size:20px;font-weight:1000;">';
    ...
    <tr style="color:darkblue"><td style="vertical-align:middle">Score&nbsp;<span class="glyphicon glyphicon-star" aria-hidden="true"></span></td><td style="vertical-align:middle">' . $s . '</td></tr>';
        ...
        while ($row = mysqli_fetch_array($q)) {
            $question = $row['qns'];
            $qid      = $row['qid'];
            $q2 = mysqli_query($con, "SELECT * FROM user_answer WHERE eid='$_GET[eid]' AND qid='$qid' AND username='$_SESSION[student_username]'") or die('Error197');
...
<tr><td style="vertical-align:middle"><b>S.N.</b></td><td style="vertical-align:middle"><b>Quiz</b></td><td style="vertical-align:middle"><b>Total Questions</b></td><td style="vertical-align:middle"><b>Right</b></td><td style="vertical-align:middle"><b>Wrong<b></td><td style="vertical-align:middle"><b>Unattempted<b></td><td style="vertical-align:middle"><b>Score</b></td><td style="vertical-align:middle"><b>Action<b></td></tr>';
    ...
    while ($row = mysqli_fetch_array($q)) {
        ...
        echo '<tr><td style="vertical-align:middle">' . $c . '</td><td style="vertical-align:middle">' . $title . '</td><td style="vertical-align:middle">' . $total . '</td><td style="vertical-align:middle">' . $r . '</td><td style="vertical-align:middle">' . $w . '</td><td style="vertical-align:middle">' . ($total - $r - $w) . '</td><td style="vertical-align:middle">' . $s . '</td><td style="vertical-align:middle"><b><a href="account.php?q=result&eid=" class="btn" style="margin:0px;background:darkred;color:white">&nbsp;<span class="title1"><b>View Result1</b></td></tr>';
    }
    echo '</table></div>';
}
if (@$_GET['q'] == 3) {
