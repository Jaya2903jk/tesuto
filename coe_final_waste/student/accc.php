...
<tr><td style="vertical-align:middle"><b>S.N.</b></td><td style="vertical-align:middle"><b>Name</b></td><td style="vertical-align:middle"><b>Total question</b></td><td style="vertical-align:middle"><b>Correct Answer</b></td><td style="vertical-align:middle"><b>Wrong Answer</b></td><td style="vertical-align:middle"><b>Total Marks</b></td><td style="vertical-align:middle"><b>Time limit</b></td><td style="vertical-align:middle"><b>Action</b></td></tr>';
  ...
  <td style="vertical-align:middle"><b><a href="account.php?q=quiz&step=2&eid=' . $eid . '&n=1&t=' . $total . '&start=start" class="btn" style="margin:0px;background:darkorange;color:white">&nbsp;<span class="title1"><b>Continue</b></span></a></b></td></tr>';
                ...
                echo '<tr style="color:darkgreen"><td style="vertical-align:middle">' . $c++ . '</td><td style="vertical-align:middle">' . $title . '&nbsp;<span title="This quiz is already solve by you" class="glyphicon glyphicon-ok" aria-hidden="true"></span></td><td style="vertical-align:middle">' . $total . '</td><td style="vertical-align:middle">+' . $correct . '</td><td style="vertical-align:middle">-' . $wrong . '</td><td style="vertical-align:middle">' . $correct * $total . '</td><td style="vertical-align:middle">' . $time . '&nbsp;min</td>
  <td style="vertical-align:middle"><b><a href="account.php?q=1" class="btn" style="margin:0px;background:darkred;color:white">&nbsp;<span class="title1"><b>View Result</b></span></a></b></td></tr>';
            }
        }
    }
    $c = 0;
...
//     echo ' <button  type="submit" class="btn btn-primary"> <a href="account.php?q=quiz&step=2&eid=' . $eid . '&t='.$count.'" style="color:white;">'.$count.'</a></button>';
...
$get_all_user_answer= mysqli_query($con,"SELECT * FROM user_answer WHERE eid='$get_exam_id' AND username='$_SESSION[username]'");
while($row_user_answer = mysqli_fetch_array($get_all_user_answer))
{

 $Popular1[] = $row_user_answer['qid'];
}
//$result=array_diff($Popular,$Popular1);

//print_r($popular);
// print_r("SELECT count(*) FROM `user_answer` inner join questions on user_answer.qid=questions.qid where username='jss'");
//$result=mysqli_query($con,"SELECT count(*) FROM `user_answer` inner join questions on user_answer.qid=questions.qid where username='jss';");
// $seatStatus = array(true, false, false, true, false, true, true, false, false, true);
// for ($seatNumber = 1; $seatNumber <= count($popular); $seatNumber++) {
//     $isBooked = $seatStatus[$seatNumber - 1];
    
//     // Define a class for the button based on the booking status
//     $buttonClass = $isBooked ? 'booked' : 'available';

//     echo '<button class="' . $buttonClass . '">Edit ' . $seatNumber . '</button>';
// }
...
for($j=0;$j<count($popular);$j++){
    $value=$popular[$j];
    $query="SELECT * FROM user_answer WHERE eid='$get_exam_id' AND username='$_SESSION[username]' AND qid='$value'";  
